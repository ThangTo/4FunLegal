"""
Bước 1: Trích xuất văn bản từ file Word (.doc/.docx).
...
'''
        ...
        print(f"  [Word COM] Thất bại: {exc}")

    try:
        output_dir = os.path.dirname(abs_doc) if "abs_doc" in locals() else os.path.dirname(doc_path)
        ...
        print(f"  [LibreOffice] Thất bại: {exc}")

    raise RuntimeError(
        "Không thể chuyển đổi .doc -> .docx tự động.\n"
        f"Vui lòng làm thủ công:\n"
        f"  1. Mở file: {doc_path}\n"
        f"  2. Save As thành: {docx_path}\n"
        "  3. Chạy lại pipeline"
    )
...
def run_extraction() -> str:
    ...
    with open(RAW_TEXT_PATH, "w", encoding="utf-8") as file_obj:
        file_obj.write(cleaned_text)
