"""
Phần 2: Xây dựng Graph Database (Neo4j).
...
Log chi tiết được ghi ra file: output/graph_build_log.txt
"""
import json
import os
...
LOG_FILE = os.path.join(OUTPUT_DIR, "graph_build_log.txt")
...
class GraphLogger:
    ...
    def _write_header(self):
        self._file.write("=" * 80 + "\n")
        self._file.write(f"  GRAPH DATABASE BUILD LOG\n")
        self._file.write(f"  Thời gian: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        self._file.write("=" * 80 + "\n\n")

    def log_node(self, node_type: str, node_id: str, number: str,
                 title: str, content_preview: str, full_path: str):
        self.node_count += 1
    ...
    def log_summary(self, stats: dict):
        ...
        for rtype, count in stats.get("relationships", {}).items():
            self._file.write(f"    {rtype}: {count}\n")
...
class GraphBuilder:
    ...
    def __init__(self, uri: str, user: str, password: str, logger: GraphLogger = None):
    ...
    def create_constraints_and_indexes(self):
        """Tạo constraints và indexes để tối ưu truy vấn."""
        constraints = [
    ...
    def load_tree_recursive(self, node_data: dict, parent_id: str = None,
                            parent_type: str = None):
        """Nạp cây phân cấp vào Neo4j đệ quy."""
    ...
    def load_cross_references(self, parsed_data: dict):
        """Tạo relationships THAM_CHIEU_TOI từ tham chiếu chéo."""
        ref_count = 0
            ...
            for ref in chunk.get("cross_references", []):
                ...
                if source_id and target_id and source_id != target_id:
                    try:
                        ...
                        if self.logger:
                            self.logger.log_crossref_skip(source_id, target_id, str(e))
...
def run_build_graph() -> dict:
    ...
    Log chi tiết được ghi ra: output/graph_build_log.txt
    """
    print("=" * 60)
    ...
    logger._file.write(f"  Stats:   {json.dumps(metadata.get('statistics', {}), indent=2)}\n")

    # Kết nối Neo4j
    print(f"[3.2] Kết nối Neo4j: {NEO4J_URL}")
    builder = GraphBuilder(NEO4J_URL, NEO4J_USER, NEO4J_PASSWORD, logger=logger)
    ...
    try:
        ...
        for i, root_node in enumerate(tree):
            ...
            print(f"  → Đã nạp root node {i + 1}/{len(tree)}: {root_node['id']}")
        ...
        print(f"  → Nodes theo loại:")
        for label, count in stats["nodes"].items():
            print(f"      {label}: {count}")
        print(f"  → Relationships theo loại:")
        ...
        print(f"\n[LOG] ✓ Log chi tiết đã ghi ra: {LOG_FILE}")
